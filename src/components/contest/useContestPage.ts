...
function useContestPage() {
	...
	const [currentPageContests, setCurrentPageContest] = useState<Contest[]>([]);
	...
	const selectableVerdictStatuses = [Verdict.SOLVED, Verdict.ATTEMPTED, Verdict.UNSOLVED];
	...
	useEffect(() => {
		if (randomContest !== -1) {
			setCurrentPageContest([contestList.contests[randomContest]]);
			return;
		}

		setCurrentPageContest(getCurrentPageContests);
	...
	useEffect(() => {
		if (!filter.canSelectMultipleCategories) {
			...
			setFilter({ ...filter, selectedCategories: [newSelectedCategory] });
		}
	...
	function updateFilter(partialFilter: Partial<Filter>) {
