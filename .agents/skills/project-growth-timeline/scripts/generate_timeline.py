...
FIELDS = ["timestamp", "main_commit", "files", "blank_lines", "comment_lines", "code_lines", "excluded_extensions", "excluded_directories"]
DATES = [dt.date(y, m, 1) for y in range(2021, 2027) for m in (2, 8)]

def run(*args):
    return subprocess.check_output(args, text=True).strip()

def measure(repo, stamp, commit):
    ...
    return [stamp.isoformat(), commit[:7], total["nFiles"], total["blank"], total["comment"], total["code"], "json", ".git;node_modules;dist;build"]

def svg(rows):
    width, height, left, right, top, bottom = 900, 620, 82, 36, 55, 70
    plot_w, plot_h = width - left - right, 190
    colors = ("#2563eb", "#16a34a")
    labels = [r[0][2:7] for r in rows]
    def panel(y, title, key, color, max_value):
        ...
        return "".join(lines)
    content = panel(55, "Non-JSON code lines", 5, colors[0], max(int(r[5]) for r in rows)) + panel(350, "Non-JSON files", 2, colors[1], max(int(r[2]) for r in rows))
...
def main():
    ...
    if args.csv.exists():
        with args.csv.open(newline="") as stream:
            for source in csv.DictReader(stream):
                existing.append([source["timestamp"], source["main_commit"], int(source["files"]), int(source["blank_lines"]), int(source["comment_lines"]), int(source["code_lines"]), source["excluded_extensions"], source["excluded_directories"]])
    ...
    for stamp in DATES:
        ...
        if commit:
            rows.append(measure(args.repo, stamp, commit))
    latest_commit = run("git", "-C", str(args.repo), "rev-parse", "origin/main")
    latest_date = dt.date.fromisoformat(run("git", "-C", str(args.repo), "show", "-s", "--format=%cs", "origin/main"))
    if latest_date.isoformat() not in known:
        rows.append(measure(args.repo, latest_date, latest_commit))
