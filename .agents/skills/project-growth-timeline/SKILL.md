...
Run `scripts/generate_timeline.py` from the repository root. It samples the latest commit on `origin/main` at six-month intervals beginning February 2021, adds the current `origin/main` tip, runs `cloc` on archived snapshots, excludes JSON files and `.git`, `node_modules`, `dist`, and `build`, and writes:
...
If the CSV already exists, preserve its rows and append only timestamps that are not already present; do not rewrite old measurements. Regenerate the SVG from all accumulated rows. Keep the CSV and timeline values synchronized. Do not use `git checkout`, rewrite branches, or count JSON as code. Verify the generated files with `git diff --check` and confirm the SVG is well-formed XML.
